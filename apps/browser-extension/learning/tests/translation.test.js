...
test("a huge raw transcript entry is split into seekable bounded segments", () => {
  const { groupTranscriptEntries } = loadSidepanelHelpers();
...
test("Kimi API requests use low-reasoning and JSON behavior", async () => {
  ...
  assert.equal(kimiRequests[0].reasoning_effort, "low");
  assert.deepEqual(kimiRequests[0].response_format, {
    type: "json_object",
  });
...
test("Kimi retries one empty transcript JSON response without response_format", async () => {
  const requests = [];
  const helpers = loadBackgroundHelpers({
    fetchImpl: async (url, options) => {
