"""
Quant Scholar Translator — Native Messaging host.

Spawned by Chrome (via launcher.bat) when the extension calls
chrome.runtime.connectNative('com.quant_scholar.translator'). Speaks Chrome's
...
class BackendManager:
    ...
    def start(self, cfg: dict) -> dict:
        ...
        if not VENV_PY.exists():
            return {"type": "error",
                    "message": f"venv python not found at {VENV_PY}. Run scripts/setup.ps1 first."}
        package_dir = PROJECT_ROOT / "quant_scholar_translator"
        if not package_dir.exists():
            return {"type": "error", "message": f"library not found at {package_dir}"}
        ...
        try:
            self.proc = subprocess.Popen(
                [str(VENV_PY), "-u", "-m", "quant_scholar_translator", "serve"],
