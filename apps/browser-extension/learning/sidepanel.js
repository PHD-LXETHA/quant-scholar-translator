 ...
 * SIDE PANEL LOGIC
 *
 * Handles the UI for Quant Scholar Translator: video detection, transcript analysis,
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  learningConfigStatus = await chrome.runtime.sendMessage({
    action: "checkConfig",
  });
...
// Keep the workspace live while captions or local ASR are still arriving.
chrome.storage?.onChanged?.addListener((changes, areaName) => {
  if (areaName !== "local" || !changes.currentLearningSession) return;
  if (currentSourceKind !== "universal") return;
  clearTimeout(navigationRefreshTimer);
  navigationRefreshTimer = setTimeout(() => checkCurrentTab(), 180);
});
...
/**
 * Reacts to the URL now in front of the panel. YouTube keeps its enhanced
 * transcript pipeline; every other site reads the unified live-learning
 * session produced by Quant Scholar's caption/ASR capture engine.
 */
...
async function checkCurrentTab() {
  try {
    ...
    debugLog("[Quant Scholar Translator Panel] Found tab:", tab?.id, tab?.url);

    if (!tab?.url) {
      await loadUnifiedLearningSession(null);
      return;
    }
    ...
    if (videoId) {
      ...
      try {
        ...
        debugLog("[Quant Scholar Translator Panel] getVideoInfo result:", result);
        ...
        console.error("[Quant Scholar Translator Panel] getVideoInfo error:", e);
        currentVideoTitle = "";
        currentChannelName = "";
...
async function loadUnifiedLearningSession(tab) {
  ...
  currentTranscriptLanguage = entries[0]?.sourceLanguage || session.sourceLanguage || null;
  ...
  currentTranscriptTimestamped = currentTranscript
...
async function startDigest(videoId, videoUrl) {
  ...
  if (!transcriptResult.success) {
    if (transcriptResult.error === "NO_LOCAL_TRANSCRIPT") {
      showError(
        "尚无字幕记录",
        "请先在 Quant Scholar Translator 弹窗中启动实时翻译，再打开学习工作台。",
      );
      return;
    }
    showError(
...
function renderAnalysisResults(analysis) {
  ...
  (analysis.chapters || []).forEach((chapter) => {
    ...
    li.addEventListener("click", () => {
      debugLog(
        "[Quant Scholar Translator Panel] Chapter clicked:",
        chapter.timestamp,
  ...
  sortedQuotes.forEach((quote) => {
    ...
    div.addEventListener("click", () => {
      debugLog(
        "[Quant Scholar Translator Panel] Quote clicked:",
        quote.timestamp,
...
async function saveQuoteAsNote(quote, btn) {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error("[Quant Scholar Translator] Save quote as note failed:", result.error);
    ...
    console.error("[Quant Scholar Translator] Save quote as note error:", error);
    btn.textContent = "Error";
    setTimeout(() => {
      btn.textContent = originalText;
      btn.disabled = false;
...
function exportTranscript() {
  ...
  exportText += `TRANSCRIPT:\n\n${transcriptContent}\n`;
  exportText += `\n${"—".repeat(60)}\n`;
  exportText += `Exported by Quant Scholar Translator\n`;
...
function showConfigError(configStatus) {
  const missingKeys = [];
  if (!configStatus.hasAiKey) missingKeys.push("Kimi");

  showState("error");
  document.getElementById("errorTitle").textContent = "API Keys Missing";
  document.getElementById("errorMessage").textContent =
    `请在 Quant Scholar Translator 设置中填写 ${missingKeys.join(" 和 ")} API Key。`;
...
async function triggerAnalysis() {
  ...
  try {
    ...
    console.error("[Quant Scholar Translator Panel] Analysis error:", error);
    if (chapterList)
...
async function seekTo(seconds) {
  debugLog("[Quant Scholar Translator Panel] seekTo called with:", seconds);
  if (seconds === undefined || seconds === null) {
    debugLog("[Quant Scholar Translator Panel] seekTo aborted - no seconds value");
    return;
  }
  ...
  try {
    if (currentSourceKind === "universal" && activePageTabId) {
      ...
      try {
        ...
        // The page may have navigated or may not expose an HTML5 media element.
      }
    }
    ...
    if (youtubeTabId) {
      try {
        ...
        debugLog("[Quant Scholar Translator Panel] seekTo direct success");
        return;
      } catch (directErr) {
        debugLog(
          "[Quant Scholar Translator Panel] Direct seekTo failed, falling back to relay:",
          directErr.message,
        );
      }
    }
    ...
    debugLog("[Quant Scholar Translator Panel] seekTo relay result:", result);
  } catch (error) {
    console.error("[Quant Scholar Translator Panel] seekTo error:", error);
  }
}
 ...
 * without consuming API tokens or external transcript-service calls.
...
async function evictOldCacheEntries(maxEntries) {
  try {
    ...
    if (toRemove.length > 0) {
      ...
      debugLog(`[Quant Scholar Translator] Evicted ${toRemove.length} old cache entries`);
    }
  } catch (error) {
...
async function loadNotes(videoId) {
  try {
    ...
    console.error("[Quant Scholar Translator Panel] Load notes error:", error);
  }
}
...
async function deleteNote(noteId) {
  try {
    ...
    console.error("[Quant Scholar Translator Panel] Delete note error:", error);
  }
}
...
async function playbackTrackingTick() {
  try {
    if (currentSourceKind === "universal" && activePageTabId) {
      const response = await chrome.tabs.sendMessage(activePageTabId, { type: "media:time" });
      if (Number.isFinite(response?.mediaTime)) highlightActiveEntry(response.mediaTime);
      return;
    }
