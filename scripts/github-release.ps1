...
$tag = 'v0.7.1'
$releaseName = 'Quant Scholar Translator 0.7.1 - Extension Hotfix'
...
try {
  ...
  $assetNames = @(
    'quant-scholar-translator-professional-0.7.1.zip',
    'quant-scholar-browser-extension-0.7.1.zip',
    'quant-scholar-safari-web-extension-0.7.1.zip',
    'quant_scholar_translator-0.7.1-py3-none-any.whl',
    'SHA256SUMS-0.7.1.txt'
  )
  if ($Mode -eq 'Publish') {
    ...
    $body = (Get-Content -LiteralPath (Join-Path $projectRoot 'docs\RELEASE_NOTES_0.7.1.md') -Raw -Encoding utf8) + "`n`n---`n`n" + (Get-Content -LiteralPath (Join-Path $projectRoot 'docs\RELEASE_NOTES_0.7.1_EN.md') -Raw -Encoding utf8)
    if (-not $release) {
