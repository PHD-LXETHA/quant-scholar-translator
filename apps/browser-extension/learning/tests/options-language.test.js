...
test("Settings copy covers English and Simplified Chinese", () => {
  assert.equal(options.translate("en", "pageTitle"), "Quant Scholar Translator Settings");
  assert.equal(options.translate("zh-CN", "pageTitle"), "Quant Scholar Translator 设置");
...
test("customization prompt switches languages and preserves technical values", () => {
  ...
  assert.match(html, /placeholder="粘贴你的 Kimi API Key"/);
  assert.match(html, /https:\/\/platform\.kimi\.com\/docs\/get-api-key/);
  assert.equal((html.match(/name="aiApiKey"/g) || []).length, 1);
  assert.doesNotMatch(html, /Transcript service API Key/i);
  assert.ok(html.includes(`>${englishPrompt}</textarea>`));
  assert.match(chinesePrompt, /^请把当前本地 Quant Scholar Translator 工作区改为使用/);
  assert.notEqual(chinesePrompt, englishPrompt);
  assert.match(
    englishPrompt,
    /Preserve local Whisper transcription, professional terminology protection, PDF reading, and knowledge export\./,
  );
  assert.match(
    chinesePrompt,
    /保留本地 Whisper、专业术语保护、PDF 阅读和知识库导出/,
  );

  for (const prompt of [englishPrompt, chinesePrompt]) {
    assert.match(prompt, /\[PROVIDER\]/);
    assert.match(prompt, /\[MODEL\]/);
    assert.match(prompt, /Chrome local storage|Chrome 本地存储/);
    assert.match(prompt, /API keys|API Key/);
