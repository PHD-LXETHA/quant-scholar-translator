...
$tag = 'v0.7.2'
$releaseName = 'Quant Scholar Translator 0.7.2 - Automatic Local Service'
...
try {
  ...
  $assetNames = @(
    'quant-scholar-translator-professional-0.7.2.zip',
    'quant-scholar-browser-extension-0.7.2.zip',
    'quant-scholar-safari-web-extension-0.7.2.zip',
    'quant_scholar_translator-0.7.2-py3-none-any.whl',
    'SHA256SUMS-0.7.2.txt'
  )
  if ($Mode -eq 'Publish') {
    ...
    $body = (Get-Content -LiteralPath (Join-Path $projectRoot 'docs\RELEASE_NOTES_0.7.2.md') -Raw -Encoding utf8) + "`n`n---`n`n" + (Get-Content -LiteralPath (Join-Path $projectRoot 'docs\RELEASE_NOTES_0.7.2_EN.md') -Raw -Encoding utf8)
    if (-not $release) {
