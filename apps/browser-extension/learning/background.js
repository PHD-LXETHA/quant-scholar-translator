...
const YTD_SETTINGS = globalThis.YTD_SETTINGS;
...
// normal Manifest V3 default. Page JavaScript still cannot access the isolated
// content-script world or extension storage APIs.
...
async function loadPromptSection(fileName, heading, variables = {}) {
  ...
  if (!markdown) {
    const response = await fetch(chrome.runtime.getURL(`learning/prompts/${fileName}`));
    if (!response.ok) {
...
async function requestAiCompletion({
  ...
  try {
    ...
    // Receiving headers proves Kimi is still making progress. Kimi
    ...
    if (typeof text !== "string" || !text.trim()) {
      const error = new Error("Kimi returned an empty response.");
    ...
    if (timeoutKind === "idle") {
      const timeoutError = new Error(
        "Kimi request was inactive for 50 seconds. Please Retry.",
      );
    ...
    if (timeoutKind === "hard") {
      const timeoutError = new Error(
        "Kimi request exceeded the 120-second limit. Please Retry.",
      );
...
async function readBoundedAiResponse(response, onActivity) {
  ...
  if (reader) {
    ...
    while (true) {
      ...
      if (done) break;
      // Every received chunk is activity, including Kimi's blank lines.
      onActivity();
      const byteLength = value?.byteLength ?? 0;
      responseBytes += byteLength;
      if (responseBytes > AI_PROVIDER_MAX_RESPONSE_BYTES) {
        await reader.cancel?.().catch(() => {});
        const error = new Error("Kimi response exceeded the 2 MiB limit.");
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "explainSelection") {
    // Explain selected text using Kimi.
  ...
  // Translation: send content to Kimi.
  ...
  if (message.action === "openSidePanel") {
    ...
    debugLog("[Quant Scholar Translator BG] openSidePanel requested from tab:", tabId);
    ...
    if (tabId) {
      ...
      chrome.sidePanel
        ...
        .catch((err) => {
          console.error("[Quant Scholar Translator BG] openSidePanel error:", err);
        });
    } else {
        ...
        .then((tabs) => {
          if (tabs[0]) {
            ...
            chrome.sidePanel.open({ tabId: tabs[0].id }).catch((err) => {
              console.error(
                "[Quant Scholar Translator BG] openSidePanel fallback error:",
                err,
              );
            });
          }
  ...
  if (message.action === "relayToContent") {
    debugLog("[Quant Scholar Translator BG] Relay request:", message.payload?.action);
    (async () => {
      try {
        ...
        debugLog(
          "[Quant Scholar Translator BG] Active tab in last focused window:",
          tabs.length,
          tabs[0]?.url,
        );
        ...
        if (!tabs[0] || !tabs[0].url?.includes("youtube.com")) {
          ...
          debugLog("[Quant Scholar Translator BG] Active YouTube tabs:", tabs.length);
        }

        // Still nothing? Try any YouTube tab
        if (!tabs[0]) {
          tabs = await chrome.tabs.query({ url: "https://www.youtube.com/*" });
          debugLog("[Quant Scholar Translator BG] Any YouTube tabs:", tabs.length);
        }

        if (tabs[0]) {
          debugLog(
            "[Quant Scholar Translator BG] Sending to tab:",
            tabs[0].id,
            "URL:",
            tabs[0].url,
          );
          ...
          debugLog("[Quant Scholar Translator BG] Got response from content:", response);
          sendResponse({ success: true, response });
        } else {
          debugLog("[Quant Scholar Translator BG] No YouTube tab found");
          sendResponse({ success: false, error: "No YouTube tab found" });
        }
      } catch (err) {
        console.error("[Quant Scholar Translator BG] Relay error:", err.message);
...
async function getPlayerVideoDetails(tabId) {
  try {
    ...
    console.warn("[Quant Scholar Translator BG] Player details unavailable:", e.message);
    return null;
  }
}

// ============================================================
// TRANSCRIPT READING FROM QUANT SCHOLAR'S LOCAL CAPTURE SESSION
// ============================================================

/**
 * Reads the timestamped learning session produced by the unified caption/ASR
 * pipeline. This removes the external transcript-service dependency: native web
 * captions are preferred and local Whisper supplies text when captions are
 * unavailable.
 */
async function handleFetchTranscript(videoId) {
  ...
  if (!segments.length || (videoId && !sessionUrl.includes(videoId))) {
    return {
      success: false,
      error: "NO_LOCAL_TRANSCRIPT",
      message:
        "请先在 Quant Scholar Translator 弹窗中启动实时翻译。系统会优先读取网页字幕，没有字幕时使用本地 Whisper。",
    };
  }
  ...
  const transcript = segments
    .map((segment, index) => {
      const wallTime = Math.max(
        0,
        ((Date.parse(segment.capturedAt || "") || startedAt) - startedAt) / 1000,
      );
      const start = Number.isFinite(Number(segment.mediaTime))
        ? Number(segment.mediaTime)
        : wallTime;
      const next = segments[index + 1];
      const nextStart = Number.isFinite(Number(next?.mediaTime))
        ? Number(next.mediaTime)
        : start + 4;
      return {
        text: String(segment.source || segment.translation || "").trim(),
        translation: String(segment.translation || "").trim(),
        start,
        duration: Math.max(0.5, nextStart - start),
        language: segment.sourceLanguage || session.sourceLanguage || null,
      };
    })
  ...
  if (!transcript.length) {
    return {
      success: false,
      error: "NO_LOCAL_TRANSCRIPT",
      message: "本地学习会话还没有可用字幕，请播放视频几秒后重试。",
    };
  }

  const transcriptText = transcript.map((segment) => segment.text).join(" ");
  const transcriptTextTimestamped = transcript
    .map((segment) => {
      const minutes = Math.floor(segment.start / 60);
      const seconds = Math.floor(segment.start % 60);
      return `[${minutes}:${String(seconds).padStart(2, "0")}] ${segment.text}`;
    })
    .join("\n");
  ...
  return {
    ...
    language: transcript[0]?.language || null,
    provider: "quant-scholar-local",
  };
}
  ...
  try {
    ...
    if (providerNeedsApiKey(settings) && !settings.aiApiKey) {
      return {
        success: false,
        error: "NO_AI_KEY",
        message: "Kimi API key not configured. Open Quant Scholar Translator Settings.",
      };
    }
    ...
    debugLog("[Quant Scholar Translator] Requesting video analysis", settings.aiModel);
    ...
    if (error.status === 429) {
      return {
        success: false,
        error: "RATE_LIMITED",
        message: "Kimi rate-limited this request. Try again shortly.",
      };
    }
    return {
      success: false,
 ...
 *
 * @param {Object} analysis - The parsed analysis from Kimi
...
/**
 * Explains selected text using Kimi.
...
async function handleSaveNote(
  ...
  try {
    ...
    // rebuilt the transcript unnecessarily on every saved note.
    let transcript = null;
    try {
      ...
      if (cached[`digest_${videoId}`]?.transcript) {
        ...
        debugLog("[Quant Scholar Translator] Using cached transcript for note");
      }
    } catch (e) {
      debugLog("[Quant Scholar Translator] No cached transcript, fetching...");
    }
    ...
    console.error("[Quant Scholar Translator] Save note error:", error);
    return { success: false, error: error.message };
  }
}

/**
 * Cleans up transcript lines using Kimi.
...
async function cleanupNoteText(
  ...
  try {
    debugLog("[Quant Scholar Translator] Requesting note cleanup");
    const variables = {
      videoTitle: videoTitle || "Unknown",
      fullContext,
    ...
    try {
      ...
      console.warn(
        "[Quant Scholar Translator] JSON parse failed for note, stripping preambles:",
        parseError,
      );
    ...
    console.error("[Quant Scholar Translator] Cleanup error:", e);
  }
...
async function handleExplainSelection(
  ...
  try {
    ...
    debugLog("[Quant Scholar Translator] Requesting selection explanation");
...
async function handleTranslateContent(
  ...
  try {
    ...
    // Kimi JSON mode can rarely return an empty content string. The prompt
    ...
    console.error("[Quant Scholar Translator] Translation error:", error);
    return { success: false, error: error.message || "Translation failed" };
  }
}

/**
 * Makes a single Kimi call for translation.
