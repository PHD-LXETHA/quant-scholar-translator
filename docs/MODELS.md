...
Whisper、NLLB 与 BabelDOC 的主要运行资源位于项目根目录 `.models`，不会依赖用户目录中的 Hugging Face 或 BabelDOC 缓存。RapidOCR 的小型 ONNX 模型随其 Python 包安装在项目 `.venv` 中。
...
| BabelDOC assets | ONNX + 字体 + CMap | 0.33 GiB | 二次 warmup 完整性校验成功 |
...
- BabelDOC DocLayout ONNX：`FECE9AF02F618B603FF7921CCEC6861D13E7E1F9830E091DFB7E8AD9311E5B21`

转换后已剔除：NLLB 原始 2.46GB PyTorch 权重、Hugging Face 构建缓存、临时 PyTorch/SymPy/mpmath 依赖、Whisper 重复下载缓存和未完成文件。保留的是实际运行所需模型、分词器、字体、CMap、CUDA 库及 Python 依赖。
