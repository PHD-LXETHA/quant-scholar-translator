...
function tryInjectNoteButton() {
  ...
  function attempt() {
    ...
    if (attempts >= maxAttempts) {
      debugLog(
        "[Quant Scholar Translator Content] Player container not found after retries, giving up",
      );
      if (ytdNoteButtonRetryTimer) {
        clearInterval(ytdNoteButtonRetryTimer);
        ytdNoteButtonRetryTimer = null;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  debugLog("[Quant Scholar Translator Content] Received message:", message.action, message);

  if (message.action === "getVideoInfo") {
    // Read video title and channel name from the page
    const info = extractVideoInfo();
    debugLog("[Quant Scholar Translator Content] Returning video info:", info);
    sendResponse(info);
  ...
  if (message.action === "seekTo") {
    ...
    debugLog("[Quant Scholar Translator Content] Seeking to:", message.seconds);
  ...
  debugLog("[Quant Scholar Translator Content] Unknown action:", message.action);
 ...
 *
 * When clicked, it opens the Quant Scholar Translator side panel.
 */
...
function createDigestButton() {
  ...
  digestButton.setAttribute("aria-label", "Open Quant Scholar Translator");
  ...
  digestButton.addEventListener("click", async (e) => {
    ...
    debugLog("[Quant Scholar Translator] Digest button clicked");
    ...
    try {
      ...
      debugLog("[Quant Scholar Translator] openSidePanel response:", result);
    } catch (err) {
      console.error("[Quant Scholar Translator] Failed to open side panel:", err);
    }
  });
...
function injectDigestButton() {
  ...
  if (!actionsContainer) {
    debugLog("[Quant Scholar Translator Content] Visible actions container not found yet");
    return false;
  }
  ...
  debugLog("[Quant Scholar Translator Content] Digest button reconciled");
  return true;
}
...
function injectNoteButton() {
  ...
  if (!playerContainer) {
    debugLog(
      "[Quant Scholar Translator Content] Player container not found yet, will retry",
    );
    return;
  }
  ...
  debugLog("[Quant Scholar Translator Content] Injecting note button");
  ...
  debugLog("[Quant Scholar Translator Content] Note button injected");
}
...
async function saveCurrentNote() {
  debugLog("[Quant Scholar Translator] Saving note");

  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[Quant Scholar Translator] No video element found");
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error("[Quant Scholar Translator] Save note error:", result.error);
    }
  } catch (err) {
    if (noteButton) {
      noteButton.innerHTML =
    ...
    console.error("[Quant Scholar Translator] Save note exception:", err);
  }
...
function seekToTimestamp(seconds) {
  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[Quant Scholar Translator Content] No video element found for seek");
    return;
  }

  debugLog("[Quant Scholar Translator Content] Seeking to:", seconds);
